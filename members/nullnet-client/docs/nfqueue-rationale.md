...
- **Runtime scope.** Sources are identified via a `bridge_ip → docker container name` cache (populated from `docker inspect`, refreshed on `docker events`). When the listener can't resolve a packet's source to a known container (pure containerd, rootless docker/podman, Kubernetes pods, host processes, Docker `--network host` containers, Swarm overlay-internal service-to-service traffic), it verdicts `ACCEPT` and logs — the packet passes through unaltered, no DNAT installed. `--network host` is a regression from the eBPF design, which fired (post-SNAT) and let the server pick *some* replica by host IP; under NFQUEUE that case no-ops cleanly instead.
